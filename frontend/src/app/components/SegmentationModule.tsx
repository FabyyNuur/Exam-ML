...
import { Users, FileText, Search, Settings, Cpu, UploadCloud, Lightbulb } from 'lucide-react';
...
const PROFILE_ACTIONS: Record<string, string> = {
  Premium: 'Programme fidélité haut de gamme, offres exclusives, cross-sell premium',
...
export function SegmentationModule() {
  ...
  const clusterNames = useMemo(() => {
    const labels = clusterMeta?.cluster_labels ?? clusterData?.metrics?.cluster_labels ?? {};
  ...
  const profileRows = useMemo(() => {
    ...
    return Object.entries(labels).map(([id, profile]) => ({
      id,
      profile,
      action: PROFILE_ACTIONS[profile] ?? 'Campagne personnalisée selon le profil',
    }));
  }, [clusterMeta]);
  ...
  const campaignData = useMemo(
    () => clusterEda?.campaign_response.map((r) => ({
      ...r,
      label: r.label === '1' ? 'Accepté' : r.label === '0' ? 'Refusé' : r.label,
    })) ?? [],
    [clusterEda],
  );
  ...
  const tabs = [
    ...
    { id: 'INTERPRETATION', label: 'INTERPRÉTABILITÉ', icon: <Lightbulb size={16} /> },
  ] as const;
  ...
  return (
    <div className="w-full h-full flex flex-col gap-6 p-6 relative z-10">
      <div className="flex flex-col gap-6 border-b border-slate-300/50 pb-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-2">Apprentissage Non Supervisé / Clustering Comportemental</p>
          </div>
      ...
      <div className="flex-1 min-h-0 relative overflow-y-auto">
        <AnimatePresence mode="wait">
          {activeTab === 'CONTEXT' && (
            <motion.div key="context" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm">
                <h3 className="text-lg font-bold text-slate-800 mb-4 border-b border-slate-100 pb-2">OBJECTIFS MARKETING</h3>
                <div className="text-slate-600 leading-relaxed space-y-4">
                  <p>
                    Identifier des profils clients homogènes pour personnaliser les campagnes marketing
                    et optimiser le retour sur investissement des actions commerciales.
                  </p>
          ...
          {activeTab === 'EDA' && (
            <motion.div key="eda" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  <h3 className="text-sm font-bold text-slate-800 mb-4 uppercase">Dépenses par canal</h3>
                  {clusterEda?.spending_by_channel?.length ? (
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={clusterEda.spending_by_channel} layout="vertical" margin={{ left: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" horizontal={false} />
                        <XAxis type="number" tick={{ fill: '#64748b', fontSize: 12 }} />
                        <YAxis type="category" dataKey="channel" tick={{ fill: '#64748b', fontSize: 11 }} width={90} />
                        <RechartsTooltip />
                        <Bar dataKey="avg" name="Dépense moy." fill="#818cf8" radius={[0, 4, 4, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  ) : (
              ...
              {campaignData.length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm max-w-md">
                  <h3 className="text-sm font-bold text-slate-800 mb-4 uppercase">Réponse aux campagnes</h3>
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={campaignData}>
                      ...
                      <YAxis tick={{ fill: '#64748b', fontSize: 12 }} />
                      <RechartsTooltip />
                      <Bar dataKey="count" name="Clients" fill="#a78bfa" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              )}
          ...
          {activeTab === 'PREPROCESSING' && (
            <motion.div key="prep" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm">
                <h3 className="text-lg font-bold text-slate-800 mb-2">{prepPage?.title}</h3>
                <p className="text-slate-500 text-sm mb-4">{prepPage?.subtitle}</p>
              </div>
              <InsightsList title="Étapes de préparation" insights={prepPage?.insights ?? []} variant="numbered" />
            </motion.div>
          )}

          {activeTab === 'MODELING' && (
            <motion.div key="modeling" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              {clusterMeta && (
                <MetricsCards
                  metrics={[
                    { label: 'Silhouette', value: clusterMeta.silhouette.toFixed(3), color: 'text-indigo-600' },
                    { label: 'Davies-Bouldin', value: clusterMeta.davies_bouldin.toFixed(3), color: 'text-indigo-500' },
                    { label: 'K optimal', value: String(clusterMeta.best_k), color: 'text-indigo-400' },
                    { label: 'Algorithme', value: clusterMeta.model_name.toUpperCase(), color: 'text-indigo-600' },
                  ]}
                />
              )}

              <InsightsList title="Modélisation & sélection de k" insights={modelPage?.insights ?? []} />
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  ...
                  <ResponsiveContainer width="100%" height={320}>
                    <ScatterChart margin={{ top: 10, right: 10, bottom: 10, left: 0 }}>
                      ...
                      <XAxis type="number" dataKey="x" name="PC1" tick={{ fill: '#64748b', fontSize: 12 }} />
                      <YAxis type="number" dataKey="y" name="PC2" tick={{ fill: '#64748b', fontSize: 12 }} />
                      <ZAxis range={[40, 160]} />
                      <RechartsTooltip cursor={{ strokeDasharray: '3 3' }} />
                      {scatterByCluster.map((group) => (
                        <Scatter key={group.name} name={group.name} data={group.data} fill={CLUSTER_COLORS[group.name] ?? '#64748b'}>
                          {group.data.map((_, i) => (
                            <Cell key={i} fill={CLUSTER_COLORS[group.name] ?? '#64748b'} />
                          ))}
                        </Scatter>
                      ))}
                    </ScatterChart>
          ...
          {activeTab === 'INTERPRETATION' && (
            <motion.div key="interpretation" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              {profileRows.length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-slate-100">
                    <h3 className="text-sm font-bold text-slate-800 uppercase">Profils clients → actions marketing</h3>
                  </div>
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-slate-600">
                      <tr>
                        <th className="text-left px-5 py-3 font-semibold">Cluster</th>
                        <th className="text-left px-5 py-3 font-semibold">Profil</th>
                        <th className="text-left px-5 py-3 font-semibold">Actions recommandées</th>
                      </tr>
                    </thead>
                    <tbody>
                      {profileRows.map((row) => (
                        <tr key={row.id} className="border-t border-slate-100">
                          ...
                          <td className="px-5 py-3 font-semibold text-indigo-700">{row.profile}</td>
      ...
      <PredictSegmentModal open={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  );
}
