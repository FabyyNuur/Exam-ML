...
from src.constants import FRAUD_FEATURE_COLUMNS, FRAUD_THRESHOLD
...
def build_class_distribution(fraud_path: str | Path) -> go.Figure:
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=["Distribution isFraud", "Fraudes par type de transaction"])
    plot_class_distribution(df["isFraud"], title="Distribution isFraud", fig=fig, row=1, col=1)
    fig.add_trace(
        go.Bar(name="Normal", x=type_fraud.index, y=type_fraud[0], marker_color=COLORS["primary"]),
        row=1, col=2,
    )
    fig.add_trace(
        go.Bar(name="Fraude", x=type_fraud.index, y=type_fraud[1], marker_color=COLORS["secondary"]),
        row=1, col=2,
    )
...
def build_amount_distribution(fraud_path: str | Path) -> go.Figure:
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=["Distribution des montants", "Montant moyen par type et classe"])
    for label, color, name in [(0, COLORS["primary"], "Normal"), (1, COLORS["secondary"], "Fraude")]:
        ...
        fig.add_trace(
            go.Histogram(
                x=amounts,
                name=name, opacity=0.7, marker_color=color, nbinsx=50,
            ),
            row=1, col=1,
        )
    fig.add_trace(go.Bar(name="Normal", x=amount_by_type.index, y=amount_by_type[0], marker_color=COLORS["primary"]), row=1, col=2)
    fig.add_trace(go.Bar(name="Fraude", x=amount_by_type.index, y=amount_by_type[1], marker_color=COLORS["secondary"]), row=1, col=2)
...
def build_suspicious_behavior(fraud_path: str | Path) -> go.Figure:
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=["Soldes émetteur : normal vs fraude", "Matrice de corrélation"])
    for label, color, name in [(0, COLORS["primary"], "Normal"), (1, COLORS["secondary"], "Fraude")]:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers", name=name, marker=dict(color=color, size=4, opacity=0.4),
            ),
            row=1, col=1,
        )
    fig.add_trace(
...
def build_roc_curves(fraud_path: str | Path, models_dir: str | Path) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(x=[0, 1], y=[0, 1], mode="lines", line=dict(dash="dash", color=COLORS["neutral"]),
...
def build_threshold_analysis(fraud_path: str | Path, models_dir: str | Path) -> go.Figure:
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=["Distribution des probabilités prédites", "F1-score en fonction du seuil"])
    fig.add_trace(
        go.Histogram(x=proba_normal, name="Normal", opacity=0.7, marker_color=COLORS["primary"], histnorm="probability density"),
        row=1, col=1,
    )
    fig.add_trace(
        go.Histogram(x=proba_fraud, name="Fraude", opacity=0.7, marker_color=COLORS["secondary"], histnorm="probability density"),
        row=1, col=1,
    )
    fig.add_vline(x=0.5, line_dash="dash", line_color="black", annotation_text="Seuil 0.5", row=1, col=1)
    fig.add_trace(
        go.Scatter(x=thresholds, y=f1_scores, mode="lines+markers", name="F1", line_color=COLORS["primary"]),
        row=1, col=2,
    )
    fig.add_vline(x=best_threshold, line_dash="dash", line_color=COLORS["secondary"],
                  annotation_text=f"Optimal={best_threshold:.2f}", row=1, col=2)
    fig.update_layout(barmode="overlay", height=450, width=1000)
...
def _shap_context(fraud_path: str | Path, models_dir: str | Path):
    ...
    return sv, X_sample, importance, base_value, y_test, X_test, X_test_scaled, explainer, inds
...
def build_shap_importance(fraud_path: str | Path, models_dir: str | Path) -> go.Figure:
    ...
    fig = go.Figure(go.Bar(x=importance.values, y=importance.index, orientation="h", marker_color=COLORS["primary"]))
    fig.update_layout(
        title="Importance des features (SHAP — valeur absolue moyenne)",
        xaxis_title="|SHAP| moyen", yaxis_title="Feature", height=600, width=900,
    )
    return fig
...
def build_shap_beeswarm(fraud_path: str | Path, models_dir: str | Path) -> go.Figure:
    ...
    for col in top_features:
        ...
        traces.append(
            go.Scatter(
                x=sv[:, idx], y=[col] * len(sv), mode="markers",
                marker=dict(size=4, opacity=0.5, color=sv[:, idx], colorscale="RdBu", showscale=False),
                showlegend=False,
            )
        )
    ...
    fig.update_layout(
        ...
        xaxis_title="Valeur SHAP", yaxis_title="Feature", height=700, width=900,
    )
    return fig


def build_shap_force_plot(fraud_path: str | Path, models_dir: str | Path) -> go.Figure:
    sv, X_sample, _, base_value, y_test, X_test, X_test_scaled, explainer, inds = _shap_context(fraud_path, models_dir)
    fraud_mask = y_test == 1
    ...
    if fraud_pos_in_test in inds:
        ...
        X_single = pd.DataFrame(X_test_scaled[fraud_pos_in_test].reshape(1, -1), columns=X_test.columns)
    ...
    fig.update_layout(
        ...
        xaxis_title="Contribution SHAP", height=600, width=900,
    )
    return fig


def all_charts(fraud_path: str | Path, models_dir: str | Path) -> dict[str, Callable[[], go.Figure]]:
